...
public class CommonJdbcCore {
	...
	/**
	 * 根据sql，模板类，绑定变量参数按分页对象查询整个结果集 查询结果集存在于分页对象中
	 **/
	...
	private Object save__(Connection con, Object obj) throws NoSuchMethodException, InstantiationException,
				...
				if (annoColumn != null && annoColumn.name() != null && annoColumn.name().length() > 0) {
					...
					if (annoColumn.id()) {
						...
						if (annoSeq != null && annoSeq.name() != null && annoSeq.name().toUpperCase().equals("SEQUENCE")
							...
							Long seqValue = queryObject(con, "SELECT " + annoSeq.sequencename() + ".nextval from dual",
									Long.class);
							args.add(seqValue);
						...
						} else if (databaseDriver.toUpperCase().equals("MYSQL") && annoSeq != null
								&& annoSeq.name() != null && annoSeq.name().toUpperCase().equals("INCREMENT")) {
							String setMethodName = "set" + methodName.substring(3, methodName.length());
	...
	private void update__(Connection con, Object obj)
	...
	private void saveOrUpdate__(Connection con, Object obj) throws InstantiationException, IllegalAccessException,
		...
		for (int i = 0; i < ms.length; i++) {
			...
			if (methodName.startsWith("get")) {
				...
				if (annoColumn != null && annoColumn.name() != null && annoColumn.name().length() > 0) {
					if (annoColumn.id()) {
						if (j > 0)
							throw new CommonJdbcException("实体存在多个ID对象！");
						j++;
						Object o = ms[i].invoke(obj);
						if (o == null) {
							save__(con, obj);
			...
			if (fs != null && fs.length > 0) {
				...
				for (int i = 0; i < fs.length; i++) {
					...
					if (in != null || out != null) {
						...
						Method method = obj.getClass().getMethod("get" + fieldName.substring(0, 1).toUpperCase()
	...
	/**
	 * 从Connection中获取PreparedStatement 并根据绑定变量执行sql查询 从ResultSet中获取分析结果集
	 * 返回泛型的list结果集
	 **/
		...
		try {
			...
			if (maxsqlcount > 0) {
				...
				if (rowcount > maxsqlcount) {
					throw new CommonJdbcException(
							"sqlcount(" + rowcount + ") is more than maxsqlcount(" + maxsqlcount + ")");
				}
				rs.beforeFirst();
			}
			// 如果执行时间大于参数设置，则抛出异常
			if (maxsqltime > 0) {
				...
				if (endtime > maxsqltime) {
					throw new CommonJdbcException(
							"sqltime(" + endtime + ") is more than maxsqltime(" + maxsqltime + ")");
				}
			}
		...
		if (args != null) {
			for (int i = 0; i < args.length; i++) {
				...
				if (arg != null) {
					...
					// 如果数据库类型Informix,Microsoft SQL
					// Server,DB2.,jConnect,SQLServer,Apache Derby
					// 中的一个则会抛出Throwable异常
					/*
					 * DatabaseMetaData dbmd = ps.getConnection().getMetaData();
					 * String databaseProductName =
					 * dbmd.getDatabaseProductName(); String jdbcDriverName =
					 * dbmd.getDriverName(); if
					 * (databaseProductName.startsWith("Informix") ||
					 * jdbcDriverName.startsWith("Microsoft SQL Server")) {
					 * useSetObject = true; } else if
					 * (databaseProductName.startsWith("DB2") ||
					 * jdbcDriverName.startsWith("jConnect") ||
					 * jdbcDriverName.startsWith("SQLServer")||
					 * jdbcDriverName.startsWith("Apache Derby")) { sqlType =
					 * Types.VARCHAR; }
					 */
				}
			}
			...
			for (int i = 0; i < args.length; i++) {
				...
				if (java.lang.String.class.equals(type)) {
					if (arg != null)
						...
						ps.setNull(i + 1, Types.VARCHAR);
				} else if (java.lang.Integer.class.equals(type) || java.lang.Byte.class.equals(type)
						|| java.lang.Short.class.equals(type) || int.class.equals(type) || byte.class.equals(type)
	 ...
	 **/
	private void processCallableStatement(CallableStatement csm, int order, int inorout, Field field, Object value)
			throws SQLException {
	 ...
	 **/
	private <T> List processResutlSet(ResultSet rs, Class<T> clazz)
			throws InstantiationException, IllegalAccessException, InvocationTargetException, SQLException {
		List<T> resultList = new ArrayList<T>();
		if (rs != null) {
			while (rs.next()) {
				if (Long.class.equals(clazz) || Integer.class.equals(clazz) || Byte.class.equals(clazz)
						|| Blob.class.equals(clazz) || Clob.class.equals(clazz) || Short.class.equals(clazz)
						...
						|| java.util.Date.class.equals(clazz) || boolean.class.equals(clazz) || long.class.equals(clazz)
						|| int.class.equals(clazz) || byte.class.equals(clazz) || short.class.equals(clazz)
	 ...
	 **/
	private void processResutlByCallabel(CallableStatement csm, Object object) throws InstantiationException,
		...
		for (int i = 0; i < fs.length; i++) {
			...
			if (in != null || out != null) {
				...
				if (out != null) {
					// 处理调用返回
					String fieldName = fs[i].getName();
					Method setMethod = object.getClass().getMethod("set" + fieldName.substring(0, 1).toUpperCase()
							+ fieldName.substring(1, fieldName.length()), fs[i].getType());
	...
	/**
	 * 从dataSource中获取数据库连接 返回获取的连接对接 返回null表示获取失败
	 **/
	...
	/**
	 * 释放数据库连接到dataSource中 返回值1：释放成功 返回值<0：释放失败
	 **/
	...
	/**
	 * 释放Statement对象 返回值1：释放成功 返回值<0：释放失败
	 **/
	...
	/**
	 * 释放PreparedStatement和ResultSet对象 返回值1：释放成功 返回值<0：释放失败
	 **/
	...
	/**
	 * 释放CallableStatement对象 返回值1：释放成功 返回值<0：释放失败
	 **/
	...
	private String processSqlPage(String sql) {
		...
		if (databaseDriver.toUpperCase().equals("MYSQL")) {
			...
			return "select * from (select page_object_.*,rownum page_rownum_ from (" + sql
					+ ") page_object_) where page_rownum_>=? and rownum<=?";
		}
	}

...
public class RmbUtil {
	...
	private final static String[] a_strNumber = { "零", "壹", "贰", "叁", "肆", "伍", "陆", "柒", "捌", "玖" };

	private final static String[] a_strModify = { "", "拾", "佰", "仟", "万", "拾", "佰", "仟", "亿", "拾", "佰", "仟" };
	...
	static private String getFraction(String pValue) {
		...
		for (int i = 0; i < maxlength; i++) {
			if (i == 0 && !("零".equals(a_strNumber[strFraction.charAt(i) - 48]))) { // 1角

				sbResult.append(a_strNumber[strFraction.charAt(i) - 48]);
				sbResult.append("角");
			} else if (i == 0 && ("零".equals(a_strNumber[strFraction.charAt(i) - 48]))) {
				// 零(角)
				sbResult.append(a_strNumber[strFraction.charAt(i) - 48]);
			} else if (i == 1 && ("零".equals(a_strNumber[strFraction.charAt(i) - 48]))) {
				// --
			} else if (i == 1 && !("零".equals(a_strNumber[strFraction.charAt(i) - 48]))) {
				// 1分
	...
	static private String getInteger(String pValue) {
		...
		// --------------------------------------------------------------
		if ("零".equals(sbResult.substring(sbResult.length() - 1)) && sbResult.length() != 1) {
	...
	static public String NumberToChinese(String pValue) {
		if ("0.00".equals(pValue)) // filter简单情况
			...
			return "零元整";
		StringBuffer sbResult = new StringBuffer(
				getSign(pValue) + getInteger(pValue) + getDot(pValue) + getFraction(pValue));
